  ...
  return (
    <>
      ...
      <SZSectionPage id="user" className="flex flex-col gap-12 md:gap-15">
        <Link
          href="/team"
          className="flex flex-row items-center gap-1.5 text-text-secondary hover:text-text-primary text-sm font-body transition-color duration-300"
        >
        ...
        <div className="flex flex-col-reverse sm:flex-row sm:items-start sm:justify-between gap-8 w-full">
          <div className="flex flex-col gap-6">
            <div className="flex flex-col gap-4">
              <div className="flex flex-row items-center gap-3 text-text-secondary font-body text-xs">
                <span className="font-mono text-sz">@{member.pseudonym}</span>
                ...
                ([key, value]) => {
                  ...
                  return (
                    <Link
                      ...
                      className="flex flex-row items-center gap-1.5 bg-bg-card hover:bg-bg-dark text-text-secondary hover:text-text-primary border border-border-card rounded-lg py-1.5 px-3.5 text-sm transition-colors duration-300"
                    >
                      <Icon size={14} />
                      {label}
                    </Link>
          ...
          <div className="shrink-0 self-start sm:self-auto">
            {member.avatar ? (
              ...
              <div
                className={`aspect-square w-32 h-32 sm:w-44 sm:h-44 md:w-50 md:h-50 rounded-3xl ${member.gradient} flex items-center justify-center font-display text-4xl md:text-[56px] font-extrabold text-white relative`}
        ...
        <div className="pt-8 border-t border-border-card w-full grid grid-cols-1 sm:grid-cols-2 gap-8">
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-row gap-2 flex-wrap">
              {languages.map((language) => (
                <TagsImage
                  key={language.name}
                  name={language.name}
                  icon={language?.icon || "/icons/stack/no-img.svg"}
                />
              ))}
            </div>
          </div>
            ...
            <div className="flex flex-row gap-2 flex-wrap">
              {tools.map((tool) => (
                <TagsImage
                  key={tool.name}
                  name={tool.name}
                  icon={tool?.icon || "/icons/stack/no-img.svg"}
                />
              ))}
            </div>
          </div>
        ...
        <div className="pt-8 border-t border-border-card w-full flex flex-col gap-6">
          <div className="flex flex-row items-center justify-between">
            <Badge
              text={`Stack & Outils (${memberProjects.length})`}
              status={false}
            />
            <Link
              href="/projects"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            {memberProjects.map((project) => {
              return <ProjectCardNoImage key={project.id} project={project} />;
            })}
          </div>
        </div>
      </SZSectionPage>
