      ...
      <SZSectionPage id="user" className="flex flex-col gap-12 md:gap-15">
        ...
        <div className="flex flex-col-reverse sm:flex-row sm:items-start sm:justify-between gap-8 w-full">
          ...
          <div className="shrink-0 self-start sm:self-auto">
            {member.avatar ? (
              <Image src={member.avatar} alt={member.name} width={200} height={200} unoptimized className="rounded-3xl w-32 h-32 sm:w-44 sm:h-44 md:w-50 md:h-50" />
            ) : (
              <div className={`aspect-square w-32 h-32 sm:w-44 sm:h-44 md:w-50 md:h-50 rounded-3xl ${member.gradient} flex items-center justify-center font-display text-4xl md:text-[56px] font-extrabold text-white relative`} style={{ boxShadow: "0 12px 48px rgba(0,0,0,0.4)" }}>
                {member.initials}
              </div>
            )}
          </div>
