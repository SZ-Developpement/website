...
export function ProjectCardHorizontal({ project }: { project: Project }) {
  return (
    <div
      key={project.id}
      className="grid grid-rows-[1fr_2fr] md:grid-rows-none md:grid-cols-[1fr_2fr] bg-bg-card border border-border-card rounded-xl overflow-hidden hover:bg-bg-dark transition-colors cursor-pointer"
    >
      <div
        className={`${project.gradient} min-w-60 flex items-center justify-center relative overflow-hidden`}
      >
        <div className="absolute inset-0 bg-[rgba(0,0,0,0.3)] backdrop-blur-sm" />
        <span className="font-display text-[80px] font-extrabold text-[rgba(255,255,255,0.15)] relative z-10">
          {project.id}
        </span>
          ...
          <div className="flex flex-row items-center gap-2">
            {project.link.github && (
              <a
                ...
                className="w-9 h-9 bg-hover-secondary border border-border-card rounded-lg flex items-center justify-center cursor-pointer text-text-secondary hover:text-text-primary transition-colors duration-300"
              >
                <GithubIcon size={15} />
              </a>
            )}
            ...
            {projectMembers.map((member, index) => (
              <Link key={member.id} href={`/team/${member.id}`}>
                <div
                  title={member.name}
                  style={{
                    backgroundImage: member.gradient.includes("bg-")
                      ? undefined
                      : member.gradient,
          ...
          <div className="flex gap-1.5">
            {project.link.github && (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center justify-center w-7.5 h-7.5 bg-hover-secondary border border-bg-hover rounded-[7px] text-text-secondary transition-colors duration-200 hover:text-white"
              >
                <GithubIcon size={13} />
              </a>
            )}
...
export const ProjectCardImage = ({ project }: { project: Project }) => {
  ...
  return (
    ...
    >
      <div
        className={`h-30 ${project.gradient} relative flex items-center justify-center overflow-hidden`}
      >
