...
export function UserCard({ member }: { member: TeamMember }) {
  ...
  return (
    <Link
      href={`/team/${member.id}`}
      className="flex flex-col gap-4 justify-between bg-bg-card border border-border-card p-6 rounded-xl hover:bg-bg-dark relative hover:translate-y-0.5 transition-all duration-500 cursor-pointer"
    >
      <ArrowUpRight
        size={16}
        color="#3a3a3a"
      ...
      <div className="flex flex-row gap-4 items-center">
        {member.avatar ? (
          ...
          <div
            className={`w-16 h-16 rounded-lg ${member.gradient} flex items-center justify-center font-display text-xl font-extrabold shrink-0 `}
      ...
      <div className="flex flex-col gap-2 ">
        <p className="text-[#6a6a6a] font-body text-xs leading-[1.7] line-clamp-2">
          {member.shortBio}
        </p>
        <div className="flex flex-row gap-1 flex-wrap">
          {member.skills.slice(0, 4).map((skill) => (
            <span
              key={skill}
              className="bg-bg-tags text-text-secondary text-[10px] font-body px-2 py-1 rounded"
            >
              {skill}
            </span>
          ))}
    ...
    <div className="flex flex-col gap-4 justify-between bg-bg-card border border-border-card p-6 rounded-xl hover:bg-bg-dark relative hover:translate-y-0.5 transition-all duration-500 cursor-pointer">
      <div className="flex flex-row gap-4 items-center">
        {member.avatar ? (
          ...
          <div
            className={`w-18 h-18 rounded-lg ${member.gradient} flex items-center justify-center font-display text-xl font-extrabold shrink-0 `}
            style={{ boxShadow: "0 8px 24px rgba(0,0,0,0.4)" }}
          >
            {member.initials}
          </div>
        ...
        <div className="flex flex-col gap-1 min-w-0">
          <h3 className="font-display text-xl font-extrabold truncate">
            {member.name}
          </h3>
          <span className="text-text-secondary font-body text-sm">
            {member.role}
