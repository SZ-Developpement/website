...
import { NavItemsAdmin } from "../../data/pages/header";
...
function HeaderAdmin() {
  ...
  return (
    <>
      <header
        className="fixed top-0 left-0 right-0 z-100 duration-200 border-b"
        style={{
      ...
      >
        <nav className="max-w-6xl px-6 mx-auto h-16 grid grid-cols-2 md:grid-cols-4 items-center">
          ...
          >
            SZ<span className="text-sz">-</span>Admin
          </Link>
          ...
          <ul className="hidden md:flex flex-row gap-8 justify-center items-center col-span-2">
            {NavItemsAdmin.map((item) => (
              <li key={item.href}>
                <Link
                  className="text-text-secondary text-sm font-normal transition-colors duration-300 font-body text-decoration-none hover:text-text-primary"
                  href={item.href}
                >
                  {item.label}
                </Link>
          ...
          <div className="flex flex-row justify-end">
            <Link
              href="#contact"
              className="hidden md:flex bg-bg-button text-bg-card px-3 py-1.5 text-[13px] rounded-lg font-semibold transition-colors duration-300 font-body text-decoration-none hover:bg-text-primary"
            >
              Ajouter un projet
            </Link>
