    ...
    >
      <div className="flex flex-col items-center justify-center gap-5">
        ...
        <p className="text-text-secondary text-body max-w-150 text-center">
          Que vous ayez une idée précise ou un simple point de départ,
          noussommes là pour vous accompagner.
        </p>
      </div>
        ...
        <div className="flex flex-col gap-4">
          {contact.map((item) => (
            <Link
              ...
              key={item.label}
              style={
                { "--hover-color": `${item.color}50` } as React.CSSProperties
              }
              className="flex flex-row items-center gap-4 bg-bg-card border border-border-card p-4 rounded-xl hover:bg-bg-dark relative hover:translate-y-0.5 transition-all duration-500 hover:border-(--hover-color) cursor-pointer"
            >
              <div
              ...
              <div className="flex flex-col gap-1">
                <h3 className="text-xs font-mono uppercase text-text-secondary">
                  {item.label}
                </h3>
          ...
          {sent ? (
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-6">
              ...
              <GroupInput
                ...
                id="subject"
                type="text"
                placeholder="Développement d'une app mobile..."
                required
                value={form.subject}
                onChange={handleChange}
              />
              ...
              <button
                type="submit"
                disabled={loading}
                className={`w-full py-2.5 rounded-lg text-sm font-semibold font-body flex items-center justify-center gap-2 ${loading ? "bg-border-card text-text-secondary cursor-all-scroll" : "bg-bg-button text-bg-card cusror-pointer hover:bg-text-primary"} transition-colors duration-300`}
              >
                {loading ? (
                  <>Envoi en cours...</>
                ) : (
